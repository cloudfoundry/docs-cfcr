...
1. Update the BOSH Director cloud config to use the new load balancer:
    <p class="terminal">$ /share/kubo-deployment/bin/deploy_bosh "${kubo_env_path}" ${private_key_filename}</p>
